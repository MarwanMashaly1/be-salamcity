...
for event in snmcEvents:
    with rate_limiter:
        ...
        db.add_event(full_description=event.get("description"), image=event.get("image"), link=event.get("link"), organization_id=5, created_at=datetime.now(), organization_name="SNMC", is_video=event.get("is_video"))
